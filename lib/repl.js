...
export default class REPL {
    ...
    doSpawn() {
        let path = this.getModuleSearchPath();
        var options = ['-XOverloadedStrings'];
        if (0 < path.length) {
            options.push('-i' + path);
        }
        this.repl = spawn(this.getGhciPath(), options);
    ...
    getModuleSearchPath() {
        let path = atom.config.get('tidalcycles.moduleSearchPath');
        return path
    }
